...
def test_pixel_shuffle_token_math():
    conn = build_connector(
        ConnectorConfig(name="pixel_shuffle", input_dim=8, output_dim=64, extra={"scale_factor": 2})
    )
...
def test_pixel_shuffle_grid_must_divide_scale():
    conn = build_connector(
        ConnectorConfig(name="pixel_shuffle", input_dim=8, output_dim=64, extra={"scale_factor": 4})
    )
